...
def build_contracts(runtime_root: Path, *, home: Path | None = None) -> dict[str, Any]:
    ...
    contracts = {
        ...
        "stage6": {
            ...
            "stage6Rehearsal": {
                "command": [
                    python,
                    rehearsal,
                    "--artifact-root", "<stage6-rehearsal-root>",
                    "--verification-manifest", "<verification-root>/verification-manifest.json",
                    "--source", "<managed-ledger-source>",
                    "--legacy-db", "<legacy-war-room-db>",
                    "--legacy-reports", "<legacy-reports-dir>",
                    "--followup", "<followup-snapshot>",
                    "--live-states", "<live-states>",
                    "--code-head", str(binding.release.get("commit")),
        ...
        "stage7": {
            "stopEvidence": {
                "command": [python, cutover, "stop-evidence", "--runtime-root", str(root), "--out", "<service-stopped-evidence>"],
                "workdir": str(root),
                "requires": ["deploy"],
            },
            "bootstrap": {
                "command": [python, cutover, "bootstrap", "--runtime-root", str(root), "--legacy-source", "<legacy-ledger-source>", "--service-stopped-evidence", "<service-stopped-evidence>", "--quiesce-token", "<quiesce-token>"],
                "workdir": str(root),
                "requires": ["stopEvidence"],
            },
            "prepare": {
                "command": [python, cutover, "prepare", "--runtime-root", str(root), "--source", "<managed-ledger-source>", "--quiesce-token", "<quiesce-token>", "--observed-at", "<observed-at>"],
                "workdir": str(root),
                "requires": ["bootstrap"],
            },
            "activate": {
                "command": [python, cutover, "activate", "--runtime-root", str(root), "--manifest", "<prepared-manifest>"],
                "workdir": str(root),
                "requires": ["restoreRehearse"],
            },
            "rollback": {
                "command": [python, cutover, "rollback", "--runtime-root", str(root), "--manifest", "<activated-manifest>"],
                "workdir": str(root),
            },
            "status": {
            ...
            "strictPreflight": {
                "command": [python, acceptance, "--runtime-root", str(root), "--managed-counts-evidence", "<managed-counts-evidence>", "--automation-snapshot", "<automation-snapshot>", "--preflight"],
                "workdir": str(root),
                "allowsWorkersLoaded": False,
                "requires": ["stageWorkerConfigs", "managedCountsEvidence", "automationSnapshot"],
            },
            "issueWorkerStagingAuthorization": {
                "command": [python, evidence, "worker-staging-authorization", "--runtime-root", str(root), "--managed-counts-evidence", "<managed-counts-evidence>"],
                "workdir": str(root),
                "outputMode": "private_0600_fixed_path",
                "requires": ["managedCountsEvidence"],
            },
            "issueOperationalAuthorization": {
                "command": [python, evidence, "operational-authorization", "--runtime-root", str(root), "--managed-counts-evidence", "<managed-counts-evidence>", "--automation-snapshot", "<automation-snapshot>"],
                "workdir": str(root),
                "outputMode": "private_0600_fixed_path",
            ...
            "strictFinalAcceptance": {
                "command": [python, acceptance, "--runtime-root", str(root), "--managed-counts-evidence", "<managed-counts-evidence>", "--automation-snapshot", "<automation-snapshot>"],
                "workdir": str(root),
                "requires": ["activateWorkers"],
            },
            "automationSnapshot": {
                "command": [python, evidence, "automation-snapshot", "--runtime-root", str(root), "--heartbeat-toml", "<heartbeat-automation-toml>", "--daily-toml", "<daily-automation-toml>", "--out", "<automation-snapshot>"],
                "workdir": str(root),
                "requires": ["automationActivation", "stageWorkerConfigs"],
            },
            "managedCountsEvidence": {
                "command": [python, evidence, "managed-counts", "--runtime-root", str(root), "--report", "<stage6-report>", "--envelope", "<stage6-envelope>", "--code-head", str(binding.release.get("commit")), "--out", "<managed-counts-evidence>"],
                "workdir": str(root),
                "requires": ["stage6Verification"],
            },
            "restoreRehearse": {
                "command": [python, cutover, "restore", "--manifest", "<prepared-manifest>", "--repo", "<source-repo>", "--mode", "rehearse"],
                "workdir": str(root),
                "requires": ["prepare"],
            },
            "restoreApply": {
                "command": [python, cutover, "restore", "--manifest", "<prepared-manifest>", "--repo", "<exact-clean-repo>", "--mode", "apply"],
                "workdir": str(root),
            },
        },
    }
    ...
    for index, step in enumerate(CUTOVER_ORDER):
        ...
        section_name, contract_name = reference.split(".", 1) if "." in reference else (None, reference)
